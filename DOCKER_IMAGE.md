# Docker image with welcome video — 2.2.1
...
This builds the tagged image `lost-movies:2.2.1`. To export it for another server:

```sh
docker save lost-movies:2.2.1 | gzip > lost-movies-2.2.1.tar.gz
```
...
```sh
docker load -i lost-movies-2.2.1-linux-amd64.tar.gz
