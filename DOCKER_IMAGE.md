# Docker image with welcome video — 2.2.0
...
This builds the tagged image `lost-movies:2.2.0`. To export it for another server:

```sh
docker save lost-movies:2.2.0 | gzip > lost-movies-2.2.0.tar.gz
```
...
```sh
docker load -i lost-movies-2.2.0-linux-amd64.tar.gz
